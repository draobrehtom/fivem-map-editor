...
menuKeybind = "DELETE"	
menuKeybind2 = "F11"	
startMessage = "¦~s~Hallo ~r~" ..GetPlayerName(PlayerId()).. "👤"
subMessage = ""
subMessage1 = "Drück ~r~[ "..menuKeybind.." ]~s~ oder ~r~[ "..menuKeybind2.." ]~s~ zum öffnen"
motd3 = "WhiteMenu v1.0~s~ ~p~~n~Discord: Lenard#5671 ~s~"
...
local assaultweapons = {
    ...
    {"WEAPON_CARBINERIFLE_MK2", "Spezi Mk II"},
    {"WEAPON_ADVANCEDRIFLE", "Advanced"},
    {"WEAPON_SPECIALCARBINE", "Spezi"},
...
local shotgunweapons = {
    ...
    {"WEAPON_SAWNOFFSHOTGUN", "Abgesaegte Pump"},
...
local function PedAttack(target, attackType)
    ...
    for k in EnumeratePeds() do
        if k ~= GetPlayerPed(target) and not IsPedAPlayer(k) and GetDistanceBetweenCoords(coords, GetEntityCoords(k)) < 2000 then
            ...
            if weparray ~= allweapons then GiveWeaponToPed(k, GetHashKey(weparray[rand][1]), 220, 0, 1)
            else GiveWeaponToPed(k, GetHashKey(weparray[rand]), 220, 0, 1) end
            ClearPedTasks(k)
...
local function DoRapidFireTick()
    ...
    if IsDisabledControlPressed(0, Keys["MOUSE1"]) then
        ...
        local targetPos = camPos + (camDir * 1.0)
...
local function GiveWeapon(target, weapon)
    local ped = GetPlayerPed(target)
    GiveWeaponToPed(ped, GetHashKey(weapon), 169, false, false)
end

local function GiveMaxAmmo(target)
    local ped = GetPlayerPed(target)
    for i = 1, #allweapons do
        AddAmmoToPed(ped, GetHashKey(allweapons[i]), 169)
    end
end
...
Citizen.CreateThread(function()
    ...
    MIOddhwuie.SetSubTitle('skid', 'By Lenard#5671')
    
    -- MAIN MENU SUBMENUS
    MIOddhwuie.CreateSubMenu('player', 'skid', 'Player Options')
    MIOddhwuie.CreateSubMenu('self', 'skid', 'Self Options')
    MIOddhwuie.CreateSubMenu('weapon', 'skid', 'Waffen')
    MIOddhwuie.CreateSubMenu('vehicle', 'skid', 'Auto')
    MIOddhwuie.CreateSubMenu('world', 'skid', 'Welt')
	MIOddhwuie.CreateSubMenu('teleport', 'skid', 'Teleport')
    MIOddhwuie.CreateSubMenu('misc', 'skid', 'Esp')
    MIOddhwuie.CreateSubMenu('lua', 'skid', 'Lua')
    ...
    MIOddhwuie.CreateSubMenu('vehiclespawner', 'vehicle', 'Auto Spawner')
    MIOddhwuie.CreateSubMenu('vehiclemods', 'vehicle', 'Auto Mods')
    MIOddhwuie.CreateSubMenu('vehiclemenu', 'vehicle', 'Auto Control Menu')
    MIOddhwuie.CreateSubMenu('VehBoostMenu', 'vehicle', 'Auto Boost Menu')
            ...
            elseif MIOddhwuie.Button('Crash FiveM ~r~AK') then
                    for i = 0, 32 do
                        ...
                        if HasModelLoaded(GetHashKey('a_m_m_afriamer_01')) then
                            ...
                            if DoesEntityExist(ped) and not IsEntityDead(GetPlayerPed(selectedPlayer)) then
                                RequestNetworkControl(ped)
                                GiveWeaponToPed(ped, GetHashKey('WEAPON_ASSAULTRIFLE'), 220, 1, 1)
                                SetPedCanSwitchWeapon(ped, true)
                                makePedHostile(ped, selectedPlayer, 0, 0)
                                TaskCombatPed(ped, GetPlayerPed(selectedPlayer), 0, 16)
                            elseif IsEntityDead(GetPlayerPed(selectedPlayer)) then
...
if MIOddhwuie.Button("~g~Reset Model To FiveM Player") then
	...
	elseif MIOddhwuie.Button("Change To ~p~Clown") then
        ...
        elseif MIOddhwuie.IsMenuOpened('weaponspawner') then
            ...
            elseif MIOddhwuie.MenuButton('Pistolen', 'pistol') then
            elseif MIOddhwuie.MenuButton('SMGs / MGs', 'smg') then
            elseif MIOddhwuie.MenuButton('Pumpguns', 'shotgun') then
            elseif MIOddhwuie.MenuButton('Sturmgewehre', 'assault') then
            elseif MIOddhwuie.MenuButton('Snipers', 'sniper') then
        ...
        elseif MIOddhwuie.IsMenuOpened('misc') then
			...
			if MIOddhwuie.MenuButton("BLIPS/ESP/Visual", 'esp') then
        ...
        elseif MIOddhwuie.IsMenuOpened('lua') then
				...
				if MIOddhwuie.MenuButton('~g~ESX ~s~& ~b~vRP ~s~💰 Moneten', 'money') then
				elseif MIOddhwuie.MenuButton('~g~ESX ~s~💼 Arbeit', 'player1') then
				elseif MIOddhwuie.MenuButton('~g~ESX ~s~🌿 Drogen', 'drogas') then
