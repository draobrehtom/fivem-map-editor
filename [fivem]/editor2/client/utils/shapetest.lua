function DoesLineIntersectAABB(p1, p2, min, max)
    if p1.x > min.x and p1.x < max.x and p1.y > min.y and p1.y < max.y and p1.z >
        min.z and p1.z < max.z then return false end
    for a in values({'x', 'y', 'z'}) do
        if (p1[a] < min[a] and p2[a] < min[a]) or
            (p1[a] > max[a] and p2[a] > max[a]) then return false end
    end
    for p in values({min, max}) do
        for a, o in pairs({x = {'y', 'z'}, y = {'x', 'z'}, z = {'x', 'y'}}) do
            local h = p1 + (p1[a] - p[a]) / (p1[a] - p2[a]) * (p2 - p1)
            local o1, o2 = o[1], o[2]
            if h[o1] >= min[o1] and h[o1] <= max[o1] and h[o2] >= min[o2] and
                h[o2] <= max[o2] then return true end
        end
    end
    return false
end
...
function RaytraceShapeTest(p1, p2, ignoredEntity)
    ...
    if hit == 0 then return nil, nil end
    if not IsEntityTargetable(entity) then return pos, nil end
    return pos, entity
end
